<template>
  <div class="container mx-auto py-8 space-y-6">
    ...
    <div v-else-if="procurement" class="space-y-6">
      ...
      <UCard>
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Submission Deadline</p>
      ...
      <UCard>
        ...
        <div class="prose dark:prose-invert max-w-none">
          <p class="whitespace-pre-wrap">{{ procurement.description }}</p>
        </div>
      </UCard>
      ...
      <UCard v-if="procurement.eligibilityCriteria">
        ...
        <div class="prose dark:prose-invert max-w-none">
          <p class="whitespace-pre-wrap">{{ procurement.eligibilityCriteria }}</p>
        </div>
      </UCard>
      ...
      <UCard v-if="procurement.contactEmail || procurement.contactPhone">
        ...
        <div class="space-y-3">
          <div v-if="procurement.contactEmail" class="flex items-center gap-2">
            ...
            <a :href="`mailto:${procurement.contactEmail}`" class="text-primary-600 hover:underline">
      ...
      <UCard v-if="getRemainingDays(procurement.submissionDeadline) >= 0">
        ...
        <div class="space-y-4">
          <p class="text-gray-600 dark:text-gray-400">
            Only registered contractors with an active status can submit bids. Please ensure you have
            your contractor number ready.
          </p>
          <UButton
            color="primary"
            size="lg"
      ...
      <UModal v-model:open="showBidForm" :ui="{ width: 'max-w-3xl' }">
        <template #header>
          ...
          <UForm :state="bidFormState" @submit="handleBidSubmit">
            <div class="space-y-4">
              ...
              <UFormField label="Technical Proposal URL" name="technicalProposalUrl">
                <UInput
                  v-model="bidFormState.technicalProposalUrl"
                  placeholder="https://..."
                />
              </UFormField>
              ...
              <UFormField label="Commercial Proposal URL" name="commercialProposalUrl">
                <UInput
                  v-model="bidFormState.commercialProposalUrl"
                  placeholder="https://..."
                />
              </UFormField>

              <UFormField label="Additional Notes" name="notes">
                <UTextarea v-model="bidFormState.notes" :rows="3" placeholder="Enter any additional notes" />
              </UFormField>
            </div>
          </UForm>
        </template>
        ...
        <template #footer>
          <div class="flex gap-2 justify-end">
            <UButton color="gray" variant="ghost" @click="showBidForm = false">
              Cancel
            </UButton>
            <UButton
              color="primary"
    ...
    <div v-else class="text-center py-12">
      ...
      <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Procurement not found</h3>
